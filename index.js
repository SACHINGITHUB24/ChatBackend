...
const allowedOrigins = [
  'http://localhost:3000',
  'https://yourfrontend.com', // replace with your frontend
  'capacitor://localhost',
  'ionic://localhost',
];
...
app.get('/api/health', async (req, res) => {
  try {
    ...
    res.json({ status: 'OK', database: 'Connected', userCount, timestamp: new Date() });
  } catch (error) {
...
app.put('/api/profile', authenticateToken, async (req, res) => {
  const { name, email, profilePic } = req.body;
  const user = await User.findByIdAndUpdate(req.user.userId, { name, email, profilePic }, { new: true, select: '-password' });
  res.json(user);
});
...
app.post('/api/upload', authenticateToken, upload.single('file'), async (req, res) => {
  ...
  const fileUrl = `${req.protocol}://${req.get('host')}/uploads/${req.file.filename}`;
  res.json({ message: 'File uploaded', url: fileUrl, filename: req.file.filename });
});

// --- Messaging ---
app.get('/api/messages/:chatId', authenticateToken, async (req, res) => {
  const { chatId } = req.params;
  const messages = await Message.find({ chatId }).populate('senderId', 'name username profilePic').sort({ timestamp: 1 });
  res.json(messages);
});

app.post('/api/messages', authenticateToken, async (req, res) => {
  const { chatId, text, type = 'text', metadata, groupId } = req.body;
  const message = await Message.create({ chatId, senderId: req.user.userId, text, type, metadata, groupId, timestamp: new Date() });
  await message.populate('senderId', 'name username profilePic');
...
app.post('/api/groups', authenticateToken, async (req, res) => {
  const { name, description, members = [] } = req.body;
  const group = await Group.create({ name, description, createdBy: req.user.userId, members: [req.user.userId, ...members], admins: [req.user.userId] });
  await group.populate('members', 'name username profilePic').populate('createdBy', 'name username');
  res.status(201).json(group);
});

// Other group endpoints (update, delete, add/remove members, get user's groups) remain same as your code above

// --- Socket.IO ---
const io = new Server(server, { cors: { origin: allowedOrigins, methods: ['GET', 'POST'], credentials: true }, allowEIO3: true });
const connectedUsers = new Map();
const activeCalls = new Map();
...
io.on('connection', (socket) => {
  ...
  socket.on('send_message', async ({ chatId, message, userId, groupId }) => {
    try {
      const newMessage = await Message.create({ chatId, senderId: userId, text: message, type: 'text', timestamp: new Date(), groupId });
      await newMessage.populate('senderId', 'name username profilePic');
      if (groupId) io.to(groupId).emit('new_message', newMessage);
      else io.emit('new_message', newMessage);
  ...
  // WebRTC signaling (call-offer, call-answer, ice-candidate, call-end, call-reject)
  socket.on('webrtc-signal', (data) => {
    const { type, callId, from, to, signal } = data;
    const target = Array.from(connectedUsers.entries()).find(([_, u]) => u.userId === to);
    if (!target) return;

    switch (type) {
      case 'call-offer':
        activeCalls.set(callId, { caller: from, callee: to, status: 'calling' });
        io.to(target[0]).emit('webrtc-signal', { type, callId, from, signal });
        break;
      case 'call-answer':
      ...
      case 'call-end':
      case 'call-reject':
        io.to(target[0]).emit('webrtc-signal', { type, callId, from });
        activeCalls.delete(callId);
        break;
    }
  });
  ...
  socket.on('disconnect', async () => {
    ...
    for (const [callId, call] of activeCalls.entries()) {
      if (call.caller === user.userId || call.callee === user.userId) {
        ...
        const other = Array.from(connectedUsers.entries()).find(([_, u]) => u.userId === otherId);
        if (other) io.to(other[0]).emit('webrtc-signal', { type: 'call-end', callId, reason: 'user_disconnected' });
        activeCalls.delete(callId);
      }
    }
